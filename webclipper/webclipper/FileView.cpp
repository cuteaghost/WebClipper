...
int CFileView::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_TreeRoot = m_wndFileView.InsertItem(L"data");//插入根节点
	strPathName += "\data";
	//MessageBox(strPathName);
	//ShowFile(L"E:\\VisualStudio\\html0.6\\html\\Debug\\data", m_TreeRoot);//以E:\\test为根目录进行遍历
	ShowFile(strPathName, m_TreeRoot);
...
void CFileView::ShowFile(CString str_Dir, HTREEITEM tree_Root)
{
	CFileFind FileFind;
	...
	while (res)
	{
		...
		if (FileFind.IsDirectory() && !FileFind.IsDots())//目录是文件夹
		{
			...
			tree_Temp = m_wndFileView.InsertItem(strTitle, 0, 0, tree_Root);
		...
		else if (!FileFind.IsDirectory() && !FileFind.IsDots())//如果是文件
		{
			...
			m_wndFileView.InsertItem(strTitle, 2, 2, tree_Temp);
		}
	}
	FileFind.Close();
}
...
void CFileView::OnExport()
{
	...
	CString filter = L"网页文件 (*.html)|*.html|文本文件 (*.txt)|*.txt||";   //文件过虑的类型  
	CFileDialog openFileDlg(isOpen, defaultDir, fileName, OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, filter, NULL);
	openFileDlg.GetOFN().lpstrInitialDir = L"E:\\FileTest\\test.doc";
	INT_PTR result = openFileDlg.DoModal();
	CString filePath = defaultDir + "\\" + fileName;
	if (result == IDOK) {
		CString oldfilename(strPathName);
		oldfilename += m_wndFileView.GetItemText(m_wndFileView.GetSelectedItem());
		CFile oldfile;
		oldfile.Open(oldfilename, CFile::modeRead, NULL);
		DWORD len = oldfile.GetLength();
		char *buf;
		buf = new char[len + 1];
		//char Buf[len + 1];
		//Buf[len] = 0;  //0终止字符串，用于输出。
		oldfile.Read(buf, len);   //Read( void* lpBuf, UINT nCount ) lpBuf是用于接收读取到的数据的Buf指针nCount是从文件读取的字节数
	...
	//刷新
	m_wndFileView.DeleteItem(m_TreeRoot);
	//获取当前路径
	GetModuleFileName(NULL, strPathName.GetBuffer(256), 256);
	strPathName.ReleaseBuffer(256);
